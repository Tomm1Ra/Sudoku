...
public class NonConsecutiveSudokuSolver {
    private Utilities utilities;
    
    public NonConsecutiveSudokuSolver(HashSet<Integer> values) {
    ...
    public HashSet<Integer> getValues(Integer []s, int r, int c, int box, int bw, int sh, int sw) {
        ...
        int cell = r*bw + c;
        for (int n : utilities.getCrossArea(cell, bw)) {
            values.remove(s[n]);
            if (s[n] != 0) {
                values.remove(s[n]+1);
                values.remove(s[n]-1);
    ...
    public static void main(String[] args)  {
        ...
        NonConsecutiveSudokuSolver nonConsecutiveSolver = new NonConsecutiveSudokuSolver(board.values);
        System.out.println("\n" + board.dumpBoard());
        System.out.println("  " + board.getFreeSlots().size() + " / " + board.getBoardSize()+"\n");
        
        board.setBlockArea(3);
        ...
        if (board.getFreeSlots().size() > 0) {
            ...
            if (board.validateSudoku2()) {
                nonConsecutiveSolver.findSolution(board.getBoard(), 0, board.getBoardWidth(), board.getBoxHeigth(), board.getBoxWidth()); 
            } 
        }
        System.out.println("\n\n" + board.dumpBoard());
