...
public class SudokuSolver {
    ...
    private boolean exclusion(SudokuBoard board) {
        System.out.print("e");
        boolean returnValue = false;
        int count = 0;
        int counter = 0;
        ...
        for (int i: board.getFreeSlots()) {
            ...
            if (board.getOptions(i).size() ==  2) {
                board.setExStart(counter);
                SudokuBoard boardA = new SudokuBoard(board.getAllValues());
                SudokuBoard boardB = new SudokuBoard(board.getAllValues());
                for (int o = 0 ; o < board.getBoardSize() ; o++) {
                    boardA.setOptions(o, board.getOptions(o));
                    boardB.setOptions(o, board.getOptions(o));
                }

                int valueA = (int)board.getOptions(i).toArray()[0];
                int valueB = (int)board.getOptions(i).toArray()[1];
                boardA.setCellValue(i, valueA);
                boardB.setCellValue(i, valueB);
                boardA.removeFreeSlots(i);
                boardB.removeFreeSlots(i);
                exclusionSolve(boardA);
                ...
                for ( int a : board.getFreeSlots() ) {
                    ...
                    for ( int m :board.getOptions(a)) {
                        if (boardA.getCellValue(a) != m && boardB.getCellValue(a) != m && !(boardA.getOptions(a).contains(m) || boardB.getOptions(a).contains(m))) {
                            returnValue = true;
                            if (count < 3) System.out.print(".");
                            blocks.add(m);
                            count++;
                        }
                        if (boardA.getCellValue(a) != 0 && boardA.getCellValue(a) == boardB.getCellValue(a)) {
                            cells.add(a);
                            values.add(boardA.getCellValue(a));
                            returnValue = true;
                            count++;
                        }
                    }
                    for (int x : blocks) board.removeOptions(a, x);
                    if (!boardA.isFilled(a) && boardA.getOptions(a).isEmpty()) {
                        board.removeOptions(i, valueA);
                    }
                    if (!boardB.isFilled(a) && boardB.getOptions(a).isEmpty()) {
    ...
    private boolean exclusion3(SudokuBoard board) {
        System.out.print("E");
        board.setExStart(0);
        boolean returnValue = false;
        int count = 0;
        ...
        for (int i: board.getFreeSlots()) {
            ...
            if (board.getOptions(i).size() ==  3) {
                board.setEx3Start(counter);
                SudokuBoard boardA = new SudokuBoard(board.getAllValues());
                SudokuBoard boardB = new SudokuBoard(board.getAllValues());
                SudokuBoard boardC = new SudokuBoard(board.getAllValues());
                for (int o = 0 ; o < board.getBoardSize() ; o++) {
                    boardA.setOptions(o, board.getOptions(o));
                    boardB.setOptions(o, board.getOptions(o));
                    boardC.setOptions(o, board.getOptions(o));
                }
                int valueA = (int)board.getOptions(i).toArray()[0];
                int valueB = (int)board.getOptions(i).toArray()[1];
                int valueC = (int)board.getOptions(i).toArray()[2];
                boardA.setCellValue(i, valueA);
                boardB.setCellValue(i, valueB);
                boardC.setCellValue(i, valueC);
                ...
                for ( int a : board.getFreeSlots() ) {
                    HashSet<Integer> blocks = new HashSet<Integer>();
                    for ( int m :board.getOptions(a)) {
                        if (boardA.getCellValue(a) != m && boardB.getCellValue(a) != m && boardC.getCellValue(a) != m && !(boardA.getOptions(a).contains(m) || boardB.getOptions(a).contains(m) || boardC.getOptions(a).contains(m))) {
                            returnValue = true;
                            blocks.add(m);
                    ...
                    if (boardA.getCellValue(a) != 0 && boardA.getCellValue(a) == boardB.getCellValue(a) && boardA.getCellValue(a) == boardC.getCellValue(a)) {
                        cells.add(a);
                        values.add(boardA.getCellValue(a));
                        returnValue = true;
                        count++;
                    ...
                    if (!boardC.isFilled(a) && boardC.getOptions(a).isEmpty()) {
    ...
    public static void main(String[] args)  {
        ...
        if (board.getFreeSlots().size() > 0) {
            ...
            if (board.validateSudoku2()) {
                solver.findSolution(board.getBoard(), 0, board.getBoardWidth(), board.getBoxHeigth(), board.getBoxWidth()); 
            }
        }
        System.out.println("\n\n" + board.dumpBoard());
