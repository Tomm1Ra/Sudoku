...
public class AntiKnightSudokuSolver {
    private Utilities utilities;
    
    public AntiKnightSudokuSolver(HashSet<Integer> values) {
    ...
    public HashSet<Integer> getValues(Integer []s, int r, int c, int box, int bw, int sh, int sw) {
        ...
        int cell = r*bw + c;
        for (int n : utilities.getKnightArea(cell, bw)) {
            values.remove(s[n]);
        }
        return values;
    }
    ...
    public static void main(String[] args)  {
        ...
        AntiKnightSudokuSolver antiKnightSolver = new AntiKnightSudokuSolver(board.values);
        System.out.println("\n" + board.dumpBoard());
        System.out.println("  " + board.getFreeSlots().size() + " / " + board.getBoardSize()+"\n");
        
        board.setBlockArea(2);
        ...
        if (board.getFreeSlots().size() > 0) {
            ...
            if (board.validateSudoku2()) {
                antiKnightSolver.findSolution(board.getBoard(), 0, board.getBoardWidth(), board.getBoxHeigth(), board.getBoxWidth()); 
            } 
        }
        System.out.println("\n\n" + board.dumpBoard());
